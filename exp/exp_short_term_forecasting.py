...
class Exp_Short_Term_Forecast(Exp_Basic):
    ...
    def _select_optimizer(self):
        if self.args.optimizer == 'Adam':
            ...
            model_optim = TS_Adam(self.model.parameters(), lr=self.args.learning_rate)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.save_each_loss and (i + 1) % 1 == 0:
                    train_loss_csv.append(loss.item())
                    val_loss_csv.append(self.vali(train_loader, vali_loader, criterion).item())
                    test_loss_csv.append(self.vali(train_loader, test_loader, criterion).item())

            if self.args.save_each_loss :
                # write csv file
                csv_file = self.args.loss_csv_fire

                # test_loss_csv = val_loss_csv
            ...
            test_loss = self.vali(train_loader, test_loader,criterion)
            # test_loss = vali_loss
    ...
    def test(self, setting, test=0):
        ...
        # result save
        folder_path = './m4_results/' + self.args.model + "_seed->"+ str(self.args.fix_seed) + '/' + self.args.optimizer + '/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        ...
        file_path = './m4_results/' + self.args.model + "_seed->"+ str(self.args.fix_seed) + '/' + self.args.optimizer + '/'
